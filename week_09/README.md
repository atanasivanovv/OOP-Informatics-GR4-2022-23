...
Сигурно сте получавали компилационни грешки от сорта на `must be a modifiable rvalue`, `lvalue` и т.н. Ще обърнем внимание на тези две понятия понеже са доста важни в С++ и също са релевантни за move семантиката. Най-просто казано, `lvalue` е стойност, която **има заделено място в паметта** (всяка променлива), докато `rvalue` е temporary стойност, за която **няма заделено място**. Могат да сравняват и в контекста на дефиниция/expression:
